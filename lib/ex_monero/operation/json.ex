defmodule ExMonero.Operation.JSON do
  @moduledoc """
...
defimpl ExMonero.Operation, for: ExMonero.Operation.JSON do
  @type response_t :: %{} | ExMonero.Request.error_t
  ...
  def perform(operation, config) do
    ...
    ExMonero.Request.request(operation.http_method, url, operation.data, headers, config, operation.service)
    |> parse(config)
  end

  def stream!(%ExMonero.Operation.JSON{stream_builder: nil}, _) do
    raise ArgumentError, """
    This operation does not support streaming!
    """
  end
  def stream!(%ExMonero.Operation.JSON{stream_builder: stream_builder}, config_overrides) do
